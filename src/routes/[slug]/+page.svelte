...
>
  ...
  <div class="body" bind:this={bodyEl}>
    ...
    {#if section.command}
      <pre class="command"><code>{section.command}</code></pre>
    {/if}
    ...
    {#if section.engineer}
      <aside class="rung rung-engineer">
        <div class="rung-label">Engineer</div>
        <p class="rung-body">{@html md(section.engineer, mdOpts)}</p>
      </aside>
    {/if}

    {#if section.manager}
      <aside class="rung rung-manager">
        <div class="rung-label">Manager</div>
        <p class="rung-body">{@html md(section.manager, mdOpts)}</p>
      </aside>
    {/if}

    {#if section.family}
      <aside class="rung rung-family">
        <div class="rung-label">Family</div>
        <p class="rung-body">{@html md(section.family, mdOpts)}</p>
      </aside>
    {/if}
...
<style>
  ...
  .rung + .rung { margin-top: 0.8rem; }

  .rung-engineer { border-left-color: var(--accent); }
  .rung-manager  { border-left-color: var(--ink); background: rgba(20, 17, 13, 0.06); }
  .rung-family   { border-left-color: var(--muted); background: rgba(20, 17, 13, 0.025); }
  ...
  .rung-manager .rung-label { color: var(--ink); }
  .rung-family  .rung-label { color: var(--muted); }
  ...
  @media (max-width: 720px) {
    ...
    .number, .title, .gesture, .body-text, .source, .rung, .command {
      grid-column: 1;
      max-width: none;
    }
    .command { margin-left: 0; }
    .title, .gesture, .body-text, .cite, .rung-body {
